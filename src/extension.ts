...
statusBarIcon.text = '$(pulse) Đang kết nối tới Discord...';
...
async function login() {
	...
	rpc.on('ready', () => {
		...
		statusBarIcon.text = '$(globe) Đã kết nối tới Discord';
		statusBarIcon.tooltip = 'Đã kết nối tới Discord';
	...
	rpc.on('disconnected', () => {
		cleanUp();
		void rpc.destroy();
		statusBarIcon.text = '$(pulse) Kết nối lại tới Discord';
		statusBarIcon.command = 'discord-kiro.reconnect';
	});
	...
	try {
		...
		if (!config[CONFIG_KEYS.SuppressNotifications]) {
			...
			if (error?.message?.includes('ENOENT'))
				void window.showErrorMessage('Không tìm thấy Discord. Hãy chắc chắn Discord đang chạy.');
			else void window.showErrorMessage(`Không thể kết nối tới Discord qua RPC: ${error as string}`);
		}
...
export async function activate(context: ExtensionContext) {
	...
	const enabler = commands.registerCommand('discord-kiro.enable', async () => {
		await disable();
		await enable();
		await window.showInformationMessage('Đã bật Discord Presence cho workspace này');
	});

	const disabler = commands.registerCommand('discord-kiro.disable', async () => {
		await disable();
		await window.showInformationMessage('Đã tắt Discord Presence cho workspace này');
	});

	const reconnecter = commands.registerCommand('discord-kiro.reconnect', async () => {
		await disable(false);
		await enable(false);
	});

	const disconnect = commands.registerCommand('discord-kiro.disconnect', async () => {
		await disable(false);
		statusBarIcon.text = '$(pulse) Kết nối lại tới Discord';
		statusBarIcon.command = 'discord-kiro.reconnect';
		statusBarIcon.show();
